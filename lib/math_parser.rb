...
class MathParser
  class << self
      ...
      while expr_idx < expr_arr.size
        ...
        case token
          ...
          when '('
            ...
            # handle if there are two consecutive nested higher precedence operations - e.g. (4 * 3) - (10 / 2)
            # it should pop '-' so the two higher precedence operations can be subtracted from each other first
            if nested_postfix.to_s.index(/[*\/]/) && nested_postfix.length == 3 # simple nested - e.g. *43
              higher_precedence = true
              higher_precedence_cnt += 1
            end

            # if higher precedence (other than two consecutive nested higher precedence)
            # pop operator
    ...
    # pop operator if two consecutive higher precedence operations
